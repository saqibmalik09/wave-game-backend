...
@Injectable()
export class TeenpattiBetQueueService implements OnModuleInit, OnModuleDestroy {
  private readonly logger = new Logger(TeenpattiBetQueueService.name);
  ...
  private async initializeQueue() {
    this.betQueue = new Queue<BetJobData>('teenpatti-bets', {
  ...
  async addBetToQueue(betData: BetJobData): Promise<{ betId: string; queuePosition: number }> {
    try {
      const job = await this.betQueue.add('place-bet', betData);
      this.stats.queued++;
  ...
  private async processBetJob(job: Job<BetJobData>): Promise<ApiResponse> {
    ...
    if (this.server) {
      this.server.to(`user:${userId}`).emit('betProcessing', {
        betId,
        status: 'processing',
    ...
    try {
      ...
      const apiResponse = await this.pQueue.add(() => this.circuitBreaker.fire(job.data));
        ...
        const potName = potIndex === 0 ? 'Pot 1' : potIndex === 1 ? 'Pot 2' : 'Pot 3';
      if (this.server) {
        this.server.to(`user:${userId}`).emit('teenpattiBetResponse', {
          ...apiResponse,
